...
public:
    ...
    bool placeMove(int row, int col, CellType symbol) {
        if(!isValid(row, col) || !board[row][col].isEmpty()) return false;
        board[row][col].setType(symbol);
        filledCells++;
        return true;
    }
    ...
    void printBoard() {
        for(int i = 0; i < rows; i++) {
            for(int j = 0; j < cols; j++) {
                CellType type = board[i][j].getType();
...
public:
    Player(int id, string name, CellType symbol) : id(id), name(name), symbol(symbol) {}
...
public:
    Game(int rows, int cols, WinningStrategy* winningStrategy) 
        : board(rows, cols), currentPlayerIndex(-1), winningStrategy(winningStrategy), status(IN_PROGRESS) {}
...
int main() {
    ...
    Player p1(1, "Chaitanya", CROSS);
    Player p2(2, "Karan", CIRCLE);
